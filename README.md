...
To address issues that touch upon several files and themes, you can add general comments based on review guidelines defined by your instructor.

1. Fill the `review-guidelines.json` file, located at the root level, with the appropriate review guidelines provided by your instructor.
2. Click on the _Reviewify_ icon to the left (red circle).
3. Write a comment for each guideline. Add a score if desired. Both will be saved continuously.
...
As previously mentioned, instructors must provide the desired review guidelines for users to insert into their `review-guidelines.json` file. Each review guideline includes a prompt that users can answer with text and an optional Likert-score. These review guidelines must be written in the following format:
